...
>
  <div class="relative mb-8 aspect-square overflow-hidden bg-[#0a0a0a]">
    ...
    {#if item.categoryName}
      <div class="absolute bottom-0 left-0 right-0 px-3 py-1.5 bg-black/60 backdrop-blur-sm">
        <span class="text-[9px] font-mono uppercase tracking-widest text-white/50">{item.categoryName}</span>
      </div>
    {/if}
  </div>
